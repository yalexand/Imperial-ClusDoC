...
function DoCGUIInitialize(varargin)
    ...
    % YA
       try
            this_dir = pwd;
            settings_fname = 'ClusDoC_settings.xml';
            [ settings, ~ ] = xml_read([this_dir filesep settings_fname]);
            handles.pixelSizenm = settings.pixelSizenm;
            handles.WindSTORM_Sigmapix = settings.WindSTORM_Sigmapix;
        catch
            disp('cannot read settings file, exiting..');
            delete(fig1);
            return
        end
    % YA
	...
	[SquareSelectIcon, ~] = imread(fullfile(currPath, 'ClusDoCUtils', 'SquareROIIcon.jpg'));
	[PolySelectIcon, ~] = imread(fullfile(currPath, 'ClusDoCUtils', 'PolyROIIcon.jpg'));
...
function Load_Data(~,~,~)
    ...
    % YA
    [fileName, pathName, filterIndex] = uigetfile( {'*.csv;*.txt'},'Select localisation table files','MultiSelect','on');
    ...
    if ismember(filterIndex, [1, 2]);
        ...
        for k = 1:length(fileName)
            ...
            if goodZENFile || contains(fileName{k},'.csv')
                importData = Import1File(fullfile(pathName, fileName{k}),handles);
