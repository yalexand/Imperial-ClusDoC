function ExportDBSCANDataToExcelFiles_YA(cellROIPair, Result, outputFolder, chan)
    ...
    Elongation_column = cell2mat(cellfun(@(x) x.Mean_Elongation, A,'UniformOutput', false));
    ...
    HeaderArray=[{'Cell'},{'ROI'},{'x bottom corner'},{'y bottom corner'},{'Size of ROI (nm)'},{'Comments'},{'Percentage of molecules in clusters'},...
        ...
        {'Circularity'}, {'Number of clusters in ROI'}, {'Density of clusters (clusters / um^2)'},{'Elongation'}];

    Matrix_Result = [Percent_in_Cluster_column*100 , Number_column(:,1) , Area_column(:,1) , Density_column*1e6 ,...
        RelativeDensity_column, TotalNumber, Circularity_column, Number_Cluster_column, Number_Cluster_column./(1e-6*cellROIPair(:,5)),Elongation_column];
    ...
    try 
        ...
        disp('error writing excel files');
