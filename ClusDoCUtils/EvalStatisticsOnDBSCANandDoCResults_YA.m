...
for i=1:column
    ...
    for j=1:row
        ...
        if ~isempty(A)
           ...
           AA=cellfun(@(x) x(x.Nb > NbThresh), A,'UniformOUtput',0);
           A=A(~cellfun('isempty', AA));
           
           % Cluster with Nb(Dof>0.4) > NbThresh      
           Cluster_DofC=cellfun(@(x) x(x.Nb_In > NbThresh), A,'UniformOUtput',0);
           ...
           % Cluster with Nb(Dof>0.4) < NbThresh
           Cluster_Other=cellfun(@(x) x(x.Nb_In <= NbThresh), A,'UniformOUtput',0);
...
switch Ch
    case 1
        ResultCh1=Result2;    
        save(fullfile(outputFolder, 'ResultCh1.mat'),'ResultCh1','-v7.3')
    case 2
        ResultCh2=Result2;    
