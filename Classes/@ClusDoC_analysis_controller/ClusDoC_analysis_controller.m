...
%%%%%%%%%%%%%%%%%%%%%%%% DoC
        ...
        'Small_Clusters_ratio_3s2s_threshold',1.7, ...
        'Small_Clusters_N2s_factor_over_most_probableN',1.7, ...
...
%
fitres = zeros(length(dr),5); % s1,s2,A1,A2,fval
...
for k=1:DN    
    ...
    fitres(k,3) = A(1);
    fitres(k,4) = A(2);        
    fitres(k,5) = fval/length(r(INTERVAL));
end
optimal_fit_ind = find(fitres(:,5)==min(fitres(:,5)));
s1 = round(fitres(optimal_fit_ind,1));
s2 = round(fitres(optimal_fit_ind,2));
A1 = round(fitres(optimal_fit_ind,3));
A2 = round(fitres(optimal_fit_ind,4));
%
out = obj.gr_assisted_quasi_cluster_analysis(u>0, u0, s1, s2, A1, A2, dir_name);
