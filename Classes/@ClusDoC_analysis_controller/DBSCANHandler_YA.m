...
try 
    ...
    %   Data = Zen format
        ...
        if DBSCANParams.UseLr_rThresh
            ...
            data(:,5) = ones(size(Data, 1), 1); % channel index            
            data(:,6) = Density;
    ...
    %                     DoCOut = Data_DoCi(Data_DoCi.DoC<0.4,:);
             ...
             if printOutFig
                ...
                print(fullfile(DBSCANParams.Outputfolder,'Cluster_maps',Name), fig1, '-dtiff');
                close(fig1);
             end
...
%                 sprintf('Ch%d', DBSCANParams.CurrentChannel), 'Cluster density maps', Name), fig2, '-dtiff');
            print(fullfile(DBSCANParams.Outputfolder,'Cluster_density_maps',Name), fig2, '-dtiff');            
            close(fig2);
