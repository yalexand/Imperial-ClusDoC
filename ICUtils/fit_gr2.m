function [g_fit, L1, L2, n1, n2, p1, p2, N1, N2, fval, F1_type, F2_type] = fit_gr(r,g_exp,N_locs,Area,mode,num_param)
                ...
                if strcmp(mode,'Gaussian')
                    F1 = @p_Gauss;
                    F2 = @p_Gauss;
                    F_type = {'Gaussian','Gaussian'};
                elseif strcmp(mode,'exponential')
                    F1 = @p_exp;
                    F2 = @p_exp;
                    F_type = {'exponential','exponential'};
                elseif strcmp(mode,'mixed')
                    F1 = @p_Gauss;
                    F2 = @p_exp;
                    F_type = {'Gaussian','exponential'};
                end      
...
%     p1
    ...
    [x,fval] = fminsearchbnd(@objfun,p0,[0 5 5 0],[inf 2000 2000 1],options);
    function y = objfun(p)
        g1 = 1 + p(1)*fun1(r,p(2));
        g2 = 1 + p(1)*fun2(r,p(3));
...
%     p1
    Nmax = 10000*length(p0);
    options = optimset('MaxFunEvals',Nmax,'MaxIter',Nmax);
    [x,fval] = fminsearchbnd(@objfun,p0,[0 5 0 5 0],[inf 2000 inf 2000 1],options);
    function y = objfun(p)
        g1 = 1 + p(1)*fun1(r,p(2));
        g2 = 1 + p(3)*fun2(r,p(4));
